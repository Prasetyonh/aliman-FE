      ...
      <div className="w-full h-auto border border-[#00000040] px-[35px]">
        <div className="mt-7 flex xxl:flex-row xl:flex-row lg:flex-row xs:flex-col md:flex-col s:flex-col justify-between">
          ...
          <div className="flex flex-row">
            ...
            <button
              ...
              className="h-[49px] w-[60px] text-[21px] text-[#535353] border border-[#00000040] rounded-r-md"
            >
              Cari
            </button>
          </div>
        ...
        {User?.length === 0 ? (
          ...
          <div className="">
                  ...
                  <div className="w-full overflow-auto">
                    <table className=" xxl:w-full xl:w-full lg:w-[800px] md:w-[800px] xs:w-[800px] s:w-[800px]">
                      <thead>
                        <tr className="grid grid-cols-5 text-left h-[57px] text-[#000000BF] text-[20px] ">
                          {tHead.map((item, idx) => (
                            <th
                              key={idx}
                              className={`col-span-${item?.span} border pl-4 border-[#00000040] flex items-center`}
                            >
                              <p>{item?.name}</p>
                            </th>
                          ))}
                      ...
                      <tbody className="bg-white">
                        {dataToShow.length === 0 ? (
                          <p>Tidak ada data dengan nama {searchValue}</p>
                        ) : (
                          <>
                            {dataToShow
                              ?.slice((count - 1) * value, count * value)
                              ?.map((item, idx) => (
                                <tr key={idx} className="grid grid-cols-5 text-gray-700 h-[48px]">
                                  ...
                                  <td className="col-span-1 flex items-center justify-center px-4  border border-[#00000040]">
                                    <div className="flex flex-row justify-center items-center gap-4">
                                      <Link
                                        to={`/admin/sunting-pengguna/${item?.id}`}
                                        state={{ id: item?.id }}
                                      >
                                        <button className="grid place-items-center rounded text-[21px] text-[#2D9CDB] border border-[#2D9CDB] px-3">
                                          <p>Edit</p>
                                        </button>
                                      </Link>
            ...
            {filteredData1.length > 5 ? (
              <div className="w-full flex justify-center mt-6">
                <nav
                  className={`${
                    number + 1 <= length
                      ? 'grid grid-cols-4 w-[214px]'
                      : 'grid grid-cols-3 w-[150px]'
                  }  place-items-center h-[46px] border-2 border-[#348FDD40] text-[24px]`}
                  aria-label="Pagination"
                >
                  <div
                    className="col-span-1 w-full h-full grid place-items-center"
                    onClick={() => btnLeft()}
                  >
                    <svg
                  ...
                  {number <= length ? (
                    <div
                      ...
                      className={`col-span-1 h-full w-full border-l-2 border-[#348FDD40] grid place-items-center ${
                        count === number ? 'bg-[#2D9CDBAB] text-[#FFFFFF]' : ''
                      }`}
                    >
                  ...
                  {number + 1 <= length ? (
                    <div
                      ...
                      className={`col-span-1 h-full w-full border-l-2 border-[#348FDD40] grid place-items-center ${
                        count === number + 1 ? 'bg-[#2D9CDBAB] text-[#FFFFFF]' : ''
                      }`}
                    >
                  ...
                  <div
                    className="col-span-1 border-l-2 border-[#348FDD40] w-full h-full grid place-items-center "
                    onClick={() => btnRight()}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
