...
export default function CardRingkasan() {
  ...
  const tHead = [{ name: 'Bulan' }, { name: 'Jumlah Pesanan' }, { name: 'Nominal' }];
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="text-left h-[57px] text-[#000000BF] text-[20px] ">
                        {tHead.map((item, idx) => (
                          <th key={idx} className="border pl-4 border-[#00000040]">
                            {item?.name}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="bg-white">
                      {sales?.sales?.map((item, idx) => (
                        <tr key={idx} className="text-gray-700 h-[48px]">
                          <td className="px-4 text-ms font-semibold border border-[#00000040]">
                            {item?.bulan}
                          </td>
                          <td className="px-4 text-ms font-semibold border border-[#00000040]">
                            {item?.jumlahPesanan}
                          </td>
                          <td className="px-4 text-sm border border-[#00000040]">
