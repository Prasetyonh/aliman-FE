...
export default function FormPesananPembeli({ productId }) {
  ...
  const handleChange = (e) => {
    ...
    // Ensure the "jumlah" field cannot be less than 0
    if (name === 'banyak') {
  ...
  useEffect(() => {
    if (voucherUser.length > 0) {
      const updatedListVoucher = voucherUser
        .map((item) => {
          const voucherUsage = {
            id: item.id,
            ...
            <div className="px-[50px] mt-6 flex flex-col gap-12 text-[24px]">
              <div className="flex flex-col gap-3 ">
                ...
                <input
                  ...
                  required
                />
                <p className="text-[16px] font-normal italic -mt-3">
                  total harga : {total <= 0 ? 0 : formatRupiah(total)}
                </p>
              </div>
              <div className="flex flex-col gap-3 ">
                <p>Gunakan Voucher</p>
                            ...
                            {({ selected }) => (
                              <>
                                <div className="flex flex-row items-center justify-between">
                                  ...
                                  {value?.jumlah > 0 ? <p>{formatRupiah(value?.jumlah)}</p> : ''}
                                </div>
                                {selected ? (
