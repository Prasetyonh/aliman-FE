...
const Role = [{ name: 'pembeli' }, { name: 'pelapak' }, { name: 'admin' }];
export default function TambahPengguna() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if "no_kk" has at least 12 digits before submitting
    if (user.no_kk.length < 16) {
      setError('No. KK minimal harus 16 angka');
      setIsOpen(true);
      return;
    }

    if (user.kontak.length < 10) {
      setError('No. HP minimal harus 10 angka');
      setIsOpen(true);
      return;
    }

    // Check if the email is in a valid format before submitting
